...
def hotspot_pairs(table: Union[str, biom.Table],
    ...
    Examples
    --------
    >>> from unifrac.gradient import hotspot_pairs
    >>> pairs = [('A', 'B'), ('B', 'C')]
    ...
    all_results = dict()
    for i, (sample_1, sample_2) in enumerate(pairs):
        ...
        all_results[(sample_1, sample_2)] = hotspot(sample_1_data,
                                                    ...
                                                    tree,
                                                    metric=metric)
    max_profiles = [result[0] for result in all_results]
    differential_abundances = [result[1] for result in all_results]
    return all_results  # max_profiles, differential_abundances
...
def hotspot(u_counts: Union[np.array, np.array],
    ...
    Examples
    --------
    >>> from unifrac.gradient import hotspot
    >>> u_counts = [0, 0, 1, 1, 2]
    >>> v_counts = [1, 1, 1, 1, 0]
    >>> otu_ids = ['a', 'b', 'c', 'd', 'e']
    >>> tree = 't1.newick'
    ...
    max_change_node, differential_abundances = _calculate_hotspot(u_counts,
                                                                  v_counts,
                                                                  otu_ids,
                                                                  tree,
                                                                  metric)
    profile = _profile_tree_node(max_change_node)
    return profile, differential_abundances
...
def _calculate_hotspot(u_counts: Union[np.array, List],
    ...
    return max_change_node, differential_abundances
...
def _profile_tree_node(node: skbio.TreeNode) -> dict:
    ...
    maximally_divergent_tips = [tip.id if tip is not None else None for tip in
                                maximally_divergent_tips]
